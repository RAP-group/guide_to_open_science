...
Importantly, the present work should be considered a compliment to the extant work fomenting open science practices in the speech sciences. 

```{r}
#| label: fig-os-flow-edit
#| out-width: 100%
#| dpi: 300
#| fig-cap: Open science practices amenable to research in linguistics as they pertain to one's stance, workflow, and the dissemination of research products.
...
That is to say, the majority of linguistic research is concentrated on specific language, mainly Indo-Germanic, in overrepresented communities, by privileged scholars. 
Making linguistic data accessible to all researchers promotes participation *in* and *with* underrepresented communities. 
...
pr1 <- pr_dat |> 
  ggplot() + 
    ...
    geom_segment(
      ...
      color = "#cc0033", linewidth = 5, show.legend = F, lineend = "round"
      ) + 
    ...
    labs(title = "OSF registrations by year", y = "Counts", x = NULL) + 
    theme_clean(base_family = "Palatino")

pr_max <- tibble(
  year = 2024.5, 
  cum = 180000, 
  text = glue("{formatC(n_pr_2024, format = 'd', big.mark = ',')} preregs\nsince 2011")
)
...
pr2 <- pr_dat |> 
  ggplot() + 
    ...
    geom_line() + 
    geom_point(color = "white", fill = "#cc0033", size = 5, pch = 21, stroke = 1) + 
    scale_x_continuous(breaks = seq(2011, 2024, 1)) + 
    scale_y_continuous(
      breaks = seq(0, 200000, 20000), 
      labels = glue("{seq(0, 200, 20)}k")
      ) + 
    coord_cartesian(ylim = c(0, 200000), xlim = c(2011, 2024)) + 
    labs(title = "Cumulative OSF registrations", y = "Cumulative counts", x = NULL) + 
    theme_clean(base_family = "Palatino") 
```
