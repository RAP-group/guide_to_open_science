...
In the early 2010's, a team of researchers in Psychology embarked on large-scale replication project to scrutinize what many considered to be the fields' major findings. <!--- embarked on A large-scale replication --->
Specifically, they attempted to replicate 100 influential studies [@open2015estimating].
The endeavor produced astounding results---of note, that approximately 53% of the major findings did not replicate---and inspired similar large-scale replication projects in other fields, yielding similar results.^[For Economics, see @camerer2016evaluating; for the Social Sciences, see @camerer2018evaluating; and, for cancer research, see @errington2021investigating.] <!--- I think this sentence can be reworded. The main idea is that only 53% of studies replicated, which is a single result (the mention of astonishing results had me anticipating the mention of something in addition to the rate of replication. --->
This is now referred to as the replication (or reproducibility) crisis. <!-- This sentence seems redundant given the first sentence in this paragraph-->
...
<!--- We mention Speech sciences for the first time here. Are we suggesting practices that are speech science specific, or could they be used in many kinds of (behavioral) linguistic studies? --->
...
For instance, "Gabriela is a white immigrant cis-gender woman from Romania whose research focuses on how non-native speakers are ideologically framed as linguistically deficient in comparison to native speakers characterized by their linguistic authority and expertise." <!--- Might be missing a few words here - consider "in comparison to native speakers who are characterized" --->
...
<!-- I think we have a great argument for the need that positionality statements are needed, but I think we could include more specific instructions about HOW one goes about creating one in terms of the conventions, default information to include etc. Do we want this to be the point of reference people use to find other points of reference or a guide in and of itself? -->

# Open data

Open data refers to information collected for the purpose of an experimental study that is as unedited as possible and is easily accessible. 
These datasets are commonly stored in internet based repositories such as the Open Science Frameworks or the IRIS database which can be accessed simply by being provided an internet link.
The datasets can be directly downloaded free of any kind of payment or exchange of information (such as an email address) by the user.
As a general rule, the data in these repositories should not contain potentially identifying information for experimental subjects, yet be as unedited as possible. 
...
Additional demographic information is often available and connected to these ids, and can be reported as if it is necessary. 

Open data is important to cumulative science for several reasons. Firstly, it allows for the analysis of the original paper to be fully reproduced and ensures that the reporting of statistical outcomes did not contain mistakes. <!--- I recall a paper or presentation by Timo that could bolster this point (that a lot of the  papers in labphon had mistakes)- do you know if I recall correctly? --->
Secondly, it allows for the same data to be analyzed in distinct ways or for future meta-anaylses. 
Statistics, like linguistics, is itself a field which advances constantly. 
Revisiting old datasets from novel statistical perspectives could enhance or contradict past narrative conclusions on the basis of collected data.
<!--- I also recall the point that we have biases as a product of incentive structure (also from Timo), but not sure we want to work this in. This might fit in with the positionality statement too -->

<!--- other thoughts:
should we make recommendations on speech data? E.g. for production data with subject's voices, should these be shared, or just the data file and PRAAT script? I feel like Joseph can speak to the best practice here. 

<!--- Ending here given my own limited resources, congrats to all on a great project and thank you for including me --->  
...
The underlying motivation for open data is relatively straightforward. 
...
In this section, we provide more detail regarding the aforementioned benefits and consider the potential challenges to open data in the field of linguistics. 
...
<!-- the open brackets i'm assuming are for a citation? -->
For instance, envision a phonetician intrigued by exploring lexical stress. 
In this scenario, the researcher might concentrate on distinct acoustic markers linked with stress, like pitch, duration, and/or intensity. 
...
The depth of preregistration directly correlates with the effort invested; the more comprehensive the preregistration, the greater the initial workload, leading to reduced effort in subsequent stages (See @fig-pr-detail). 
